...
def analyze():
    ...
    with open("C:/Users/Ibrahim.Moghul/Desktop/Data Analysis Scripts/OUTPUT/Chambers/test.csv", 'w') as out:
        ...
        for fn in bar:
            ...
            w = plt.subplot(grid[0, 0:1])#plt.subplot(2,iterations,1)
            ...
            e = plt.subplot(grid[0, 1:3],sharex = w,sharey = w)#plt.subplot(2,iterations,3)
            e.plot(t, interest["Watt"], "paleturquoise")
            e.plot(t, watts, "blue")
            for i in getLinearParts(t,watts):
                # print(i)
                e.plot( t[i[0]:i[1]+1] ,watts[i[0]:i[1]+1],color="green",linewidth = 3)
            plt.title("processed watts")

            d = plt.subplot(grid[0, 3],sharex = w)
